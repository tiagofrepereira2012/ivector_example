...
def main():
  ...
  parser.add_argument('-p', '--protocol', metavar='PROTOCOL', type=str, dest="protocol", help='The REPLAY-ATTACK protocol type may be specified instead of the the id switch to subselect a smaller number of files to operate on', choices=protocols)
  ...
  for key, filename in process.items():
    ...
    #loading five quantities
    fiveq = bob.io.load(fiveqdict[key])
    fiveq = numpy.average(fiveq,axis=0)
